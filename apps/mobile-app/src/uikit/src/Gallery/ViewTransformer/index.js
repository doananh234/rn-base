...
export default class ViewTransformer extends React.Component {
  ...
  onResponderRelease = (evt, gestureState) => {
    const handled = this.props.onTransformGestureReleased
    ...
    if (gestureState.doubleTapUp) {
      ...
      this.state.scale === 1 && this.performFling(gestureState.vx, gestureState.vy);
    } else {
      this.animateBounce();
    }
  };
  ...
  performFling = (vx, vy) => {
    ...
    let maxY;
    let minY;
    const availablePanDistance = availableTranslateSpace(
